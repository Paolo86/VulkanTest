...
Texture2D::Texture2D(uint32_t width, uint32_t height, uint32_t channels, void* imageData)
{
	...
	m_image = VkUtils::ImageUtils::CreateImage(Vk::Instance().m_physicalDevice, Vk::Instance().m_device,width, height, 
		VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_TILING_OPTIMAL, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, &m_imageMemory);
	...
	VkUtils::ImageUtils::TransitionImageLayout(Vk::Instance().m_device, Vk::Instance().m_graphicsQ, Vk::Instance().m_commandPool, m_image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);

	vkDestroyBuffer(Vk::Instance().m_device, staging, nullptr);
	vkFreeMemory(Vk::Instance().m_device, stagingMemory, nullptr);

	m_imageView = VkUtils::ImageUtils::CreateImageView(Vk::Instance().m_device, m_image, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_ASPECT_COLOR_BIT);
}
